...
class ControllerCatalogProduct extends Controller {
	...
	public function filter() {
  	...
  	protected function getForm() {
		...
		$this->data['entry_memory'] = $this->language->get('entry_memory');
		$this->data['entry_product_color'] = $this->language->get('entry_product_color');
		...
		if (!isset($this->request->get['product_id'])) {
			...
			$this->data['update'] = $this->url->link('catalog/product/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
			$this->data['preview'] = $this->url->link('product/product', 'product_id=' . $this->request->get['product_id']);
		}
		...
		if (isset($this->request->post['product_color_value'])) {
			$this->data['product_color_value'] = $this->request->post['product_color_value'];
		} elseif (!empty($product_info)) {
			$this->data['product_color_value'] = $product_info['product_color_value'];
		} else {
			$this->data['product_color_value'] = '';
		}
	
		if (isset($this->request->post['product_memory_value'])) {
			$this->data['product_memory_value'] = $this->request->post['product_memory_value'];
		} elseif (!empty($product_info)) {
			$this->data['product_memory_value'] = $product_info['product_memory_value'];
		} else {
			$this->data['product_memory_value'] = '';
		}
		...
		*/
		...
		if (isset($this->request->post['product_color'])) {
			$products = $this->request->post['product_color'];
		} elseif (isset($this->request->get['product_id'])) {		
			$products = $this->model_catalog_product->getProductColor($this->request->get['product_id']);
		} else {
			$products = array();
		}
		...
		foreach ($products as $product_id) {
			$color_info = $this->model_catalog_product->getProduct($product_id);
			
			if ($color_info) {
				$this->data['product_color'][] = array(
					'product_id' => $color_info['product_id'],
					'name'       => $color_info['name']
				);
			}
		}

		if (isset($this->request->post['product_memory'])) {
			$products = $this->request->post['product_memory'];
		} elseif (isset($this->request->get['product_id'])) {		
			$products = $this->model_catalog_product->getProductMemory($this->request->get['product_id']);
		} else {
			$products = array();
		}
	
		$this->data['product_memory'] = array();
		
		foreach ($products as $product_id) {
			$memory_info = $this->model_catalog_product->getProduct($product_id);
			
			if ($memory_info) {
				$this->data['product_memory'][] = array(
					'product_id' => $memory_info['product_id'],
