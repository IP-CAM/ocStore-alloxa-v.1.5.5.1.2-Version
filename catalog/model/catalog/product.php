...
//		$product_data['sql']=$sql;
	...
	public function getProductAttributes($product_id) {
		...
		foreach ($product_attribute_group_query->rows as $product_attribute_group) {
			...
			foreach ($product_attribute_query->rows as $product_attribute) {
				if (strpos($product_attribute['text'], 'Есть') !== false) {
					$product_attribute['text'] = str_replace('Есть', '<img src="catalog/view/theme/default/images/td_yes.jpg" title="Есть" />', $product_attribute['text']);
				} elseif (strpos($product_attribute['text'], 'Нет') !== false) {
					$product_attribute['text'] = str_replace('Нет', '<img src="catalog/view/theme/default/images/td_no.jpg" title="Нет" />', $product_attribute['text']);
				}
				$product_attribute_data[] = array(
					'attribute_id' => $product_attribute['attribute_id'],
					'name'         => $product_attribute['name'],
					'text'         => str_replace('++', '<br />', str_replace('\"', '"', $product_attribute['text']))
				);
			}
	...
	public function getShortDescription($product_id) {
		$product_attribute_query = $this->db->query("SELECT a.attribute_id, ad.name, ad.f_name, pa.text, pa.f_text FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_group ap ON (a.attribute_group_id = ap.attribute_group_id) WHERE pa.product_id = '" . (int)$product_id . "' AND pa.short = 1 AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pa.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ap.sort_order, a.sort_order");
		...
		foreach ($product_attribute_query->rows as $product_attribute) {
			$product_attribute_data[] = array(
				'attribute_id' => $product_attribute['attribute_id'],
				'name'         => !empty($product_attribute['f_name']) ? $product_attribute['f_name'] : $product_attribute['name'],
				'text'         => !empty($product_attribute['f_text']) ? str_replace('++', '<br />', str_replace('\"', '"', $product_attribute['f_text'])) : str_replace('++', '<br />', str_replace('\"', '"', $product_attribute['text']))	 	
			);
		}
	...
	public function getProductColor($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_color pc LEFT JOIN " . DB_PREFIX . "product p ON (pc.color_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pc.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
		
		foreach ($query->rows as $result) { 
			if( $product_id != $result['color_id'] )$product_data[$result['color_id']] = $this->getProduct($result['color_id']);
		}
	...
	public function getProductMemory($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_memory pm LEFT JOIN " . DB_PREFIX . "product p ON (pm.memory_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pm.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
		
		foreach ($query->rows as $result) { 
			if( $product_id != $result['memory_id'] )$product_data[$result['memory_id']] = $this->getProduct($result['memory_id']);
		}
	...
	public function getMainCategory($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE main_category = 1 AND product_id = " . $product_id);
	...
	public function buildPath($category_id, $path = '') {
		...
		if ($query->row['parent_id'] != 0) {
			...
			return $this->buildPath($query->row['parent_id'], $path);
		} else {
			if (empty($path)) {
				$path = $query->row['category_id'];
			} else {
