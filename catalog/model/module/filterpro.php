...
class ModelModuleFilterPro extends Model {
	...
	public function getAttributes($data) {
		...
		$sql = "SELECT DISTINCT pa.text, a.`attribute_id`, ad.`name`, ad.`f_name`, ag.attribute_group_id, agd.name as attribute_group_name FROM `" . DB_PREFIX . "product_attribute` pa" .
		...
		foreach($query->rows as $row) {
			...
			if(!isset($attributes[$row['attribute_group_id']]['attribute_values'][$row['attribute_id']])) {
				if (!empty($row['f_name'])) {
					$attributes[$row['attribute_group_id']]['attribute_values'][$row['attribute_id']] = array('name' => $row['f_name'], 'values' => array());
				} else {
