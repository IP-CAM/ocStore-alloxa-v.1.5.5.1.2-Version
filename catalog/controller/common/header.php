...
class ControllerCommonHeader extends Controller {
	protected function index() {
		...
		$menu_categories = $this->cache->get('menu_categories');
		...
		if (!$menu_categories) {
					...
					foreach ($children as $child) {
						...
						//Будем вычислять кол-во товаров в категориях только если это кол-во надо показывать
							if ($this->config->get('config_product_count')) {
								...
								$product_child_total = $this->model_catalog_product->getTotalProducts($data_child);
							}
											
							$children_children_data[] = array(
								'name'  => $child_child['name'] . ($this->config->get('config_product_count') ? ' (' . $product_child_total . ')' : ''),
								'line'	=> $child_child['line'],
								'line_on' => $child_child['line_on'],
			...
			$this->cache->set('menu_categories', $this->data['categories']);

		} else {

			$this->data['categories'] = $menu_categories;
