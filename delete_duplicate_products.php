...
$products_in_store_query = $db->query("select query, keyword from oc_url_alias where query like 'product_id=%'");
$products_in_store = array();

foreach ($products_in_store_query->rows as $product) {
	if (in_array($product['keyword'], $products_in_store)) {
 		deleteProduct((int)str_replace('product_id=', '', $product['query']));
 	} else {
 		$products_in_store[] = $product['keyword'];
 	}
}
