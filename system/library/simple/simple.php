    ...
    public function html_field($field) {
        ...
        if ($field['type'] == 'select' || $field['type'] == 'select_from_api') {
			...
			if($field['id'] == 'main_zone_id')
				$html = '<select '.(!empty($field['error']) ? ' class="simplecheckout-red-border"' : '').' id="'.($field['from']).'_'.$field['id'].'" name="'.$field['from'].'['.$field['id'].']"'.($field['id'] == 'main_zone_id' ? ' onchange="$(\'#'.($field['from']).'_main_city\').load(\'index.php?route=checkout/simplecheckout_customer/city&zone_id=\' + this.value);"' : '').($field['reload'] ? ' reload="'.$field['from'].'_changed"' : '').'>';
