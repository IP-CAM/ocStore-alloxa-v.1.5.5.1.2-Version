...
class Pagination {
	...
	public function render() {
		...
		$this->url = str_replace('{limit}', $limit, $this->url);
		$limits = '<select id="list-limit">';
		for ($i = 0; $i <= 500; $i+=10) {
				$limits .= '<option value="'. str_replace('{page}', $page, $this->url) .'" '.($i==$limit? 'selected' : '').'>'. ($i==0?'Все':$i) .'</option>';
		}
		$limits .= '</select>';
		
		return ($output ? '<div class="' . $this->style_links . '">' . $output . '</div>' : '') . '<div class="' . $this->style_results . '">Показать по ' . $limits . ' ' . str_replace($find, $replace, $this->text) . '</div>';
	}
}
?>
